...
- Task 2 do plano Assets/Render: 5 faixas de música fixas reais em `render-service`'s `/files/music/track{1..5}.mp3` (fonte incompetech.com, CC-BY).
- **Atribuição de música (Kevin MacLeod/incompetech.com, CC-BY 4.0) adicionada na descrição dos 2 uploads do Publish** (`n8n-workflows/publish.json`, nodes "Upload 16:9 to YouTube" e "Upload 9:16 to YouTube") — aplicado em 2026-07-20 direto na instância viva via `PUT /api/v1/workflows/WzykicrsWM9N0Wqi` (GET prévio pra preservar credenciais reais e `settings` restrito ao subconjunto aceito pelo PUT — ver "Achados de metodologia" em `n8n-instance.local.md`).
